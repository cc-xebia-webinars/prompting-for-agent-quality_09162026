...
Report, in this order: correctness risks, convention violations (especially any use of the legacy fee helper outside `Domain/Legacy/` and the reconciliation job), missing tests, and readability. Quote file paths and line numbers. Do not edit files. End with a one-line verdict: approve, approve with nits, or request changes.
