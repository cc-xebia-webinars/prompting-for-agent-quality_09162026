...
PASSED = re.compile(r"\b\d+ passed\b|Tests\s+\d+ passed|BUILD SUCCESS|Passed!|succeeded: \d+")
...
PRESENTER_FILES = re.compile(r"tier[-_]?check|RUN_COMPARISON|run-stats\.py", re.IGNORECASE)
...
def find_session(session_id: str | None) -> Path | None:
    root = Path(os.environ.get("COPILOT_HOME", Path.home() / ".copilot")) / "session-state"
    if not root.is_dir():
        return None
    if session_id:
...
def session_numbers(events: list[dict]) -> dict:
    ...
    for event in events:
        ...
        if kind == "session.start" and data.get("selectedModel"):
            ...
            if not start or start["name"] not in SHELL_TOOLS or not data.get("success", True):
                continue
    ...
    numbers["credits"] = round(nano / 1e9, 2) if isinstance(nano, (int, float)) else None
...
def vscode_workspace_roots() -> list[Path]:
    ...
    return [base / name / "User" / "workspaceStorage" for base in bases for name in names]
    ...
    for root in vscode_workspace_roots():
        ...
        for workspace in root.iterdir():
            ...
            with contextlib.suppress(OSError, json.JSONDecodeError):
                ...
                if not folder.startswith("file:") or normalized(file_uri_path(folder)) != here:
                    continue
...
def vscode_numbers(state: dict) -> dict:
    ...
    for request in state.get("requests") or []:
        ...
        for item in request.get("response") or []:
            ...
            numbers["tool_calls"] += 1
            numbers["subagent_tool_calls"] += 1 if item.get("subAgentInvocationId") else 0
            ...
            if VSCODE_EDIT_TOOL.search(tool):
                ...
                label = invocation.get("value") if isinstance(invocation, dict) else invocation
            ...
            if specific.get("kind") == "subagent" and TEST_COMMAND.search(str(specific)):
...
def print_vscode(path: Path, numbers: dict) -> None:
    ...
    print(
        f"VS Code chat session {path.stem}" + (f": {numbers['title']}" if numbers["title"] else "")
    )
    ...
    for index, r in enumerate(requests, start=1):
        credit = f"{r['credits']:.1f} credits" if isinstance(r["credits"], (int, float)) else "n/a"
        print(
    ...
    print(f"- Clarifying questions asked: {'yes' if questions else 'no'}{listed(questions)}")
    ...
    for path in git("ls-files", "--others", "--exclude-standard").splitlines():
        if TEST_FILE.search(path) and not PRESENTER_FILES.search(path):
            with contextlib.suppress(OSError):
                ...
                tests_added += sum(1 for row in text.splitlines() if TEST_DECLARATION.search(row))
...
def warn_answer_files(paths: list[str]) -> None:
    ...
    if seen:
        print(f"  WARNING: the agent read presenter files{listed(seen)}. They hold the expected")
        print("  answers, so this run is not a fair sample.")


def print_session(folder: Path, numbers: dict) -> None:
    open_note = "" if numbers["closed"] else " (still open: credits so far, tokens after /exit)"
    ...
    print(f"- Tool calls: {numbers['tool_calls']}{listed([f'{k} {v}' for k, v in tools])}")
    if runs:
        print(f"- Test runs by the agent: {len(runs)} (first: {runs[0]}, last: {runs[-1]})")
    else:
    ...
    written = numbers["files_written"]
    size = (
        f", +{lines_changed[0]} -{lines_changed[1]} lines" if lines_changed[0] is not None else ""
    )
    print(f"- Files the agent edited: {len(written)}{listed(written)}{size}")
    tokens = ", ".join(f"{key} {number(value)}" for key, value in numbers["tokens"].items())
...
def main() -> int:
    ...
    parser.add_argument("--session", help="session id (default: newest session in this folder)")
    parser.add_argument("--vscode", action="store_true", help="read the VS Code chat session")
    parser.add_argument("--git-only", action="store_true", help="only the repository numbers")
    args = parser.parse_args()

    if args.vscode and not args.git_only:
        chat = find_vscode_session(args.session)
        if chat is None:
            print("No VS Code chat session found for this folder. Run this from the folder that")
            print("is open in VS Code, or pass --session <id> (a file name under chatSessions).")
            return 1
        print_vscode(chat, vscode_numbers(replay_vscode_session(chat)))
    elif not args.git_only:
        folder = find_session(args.session)
        if folder is None:
            print("No Copilot CLI session found for this folder. Run this from the folder you")
            print("started copilot in, or pass --session <id> from the 'Resume' line at /exit.")
            return 1
    ...
    print("Not covered: the quote total (run the quote check) and the suite now (run the tests).")
    return 0
