...
- **Commands** are the same on all three systems unless the script shows a separate
  **macOS and Linux** block and a **Windows** block. Paths are written as `~/demos/...`; in
  PowerShell `~` is your user folder too, so `cd ~/demos/csharp-feequote` works everywhere.
...
1. Install git, VS Code with the GitHub Copilot, Copilot Chat and C# Dev Kit extensions,
   Copilot CLI and the .NET 10 SDK (<https://dotnet.microsoft.com/download>).
   - **macOS:** `brew install git` and `brew install --cask dotnet-sdk copilot-cli` (and the
     Xcode command line tools if git asks for them).
   - **Linux:** git and the .NET 10 SDK from your package manager (for example
     `sudo apt-get install -y git dotnet-sdk-10.0` on Ubuntu), and
     `npm install -g @github/copilot`.
   - **Windows:** `winget install Git.Git Microsoft.DotNet.SDK.10 GitHub.Copilot Microsoft.PowerShell`.
     Windows PowerShell 5.1 can run the setup script, but every other command in this script
     needs PowerShell 7.
2. Copy two folders out of any synchronized folder (OneDrive, iCloud Drive, Dropbox) into
   `~/demos` (on Windows that is `C:\Users\<you>\demos`), side by side: the `csharp-feequote`
   ...
   below assume `~/demos/csharp-feequote` and `~/demos/demo-kit`; if yours differ, change
...
3. In a terminal in that folder, run:
   ...
   ```bash
   dotnet restore
   bash ../demo-kit/csharp-feequote/setup-demo.sh
   ```
   ...
   ```powershell
   dotnet restore
   ..\demo-kit\csharp-feequote\setup-demo.ps1
   ```

   If Windows blocks the script, run
   `pwsh -ExecutionPolicy Bypass -File ..\demo-kit\csharp-feequote\setup-demo.ps1`.
   ...
   | `demo-checks` branch, tag `demo-checks-base` | `main` plus one commit with a defect the tests and the build do not catch but `dotnet format` does. Used by 4.7. The tag lets you restore the branch after an experiment. |

   Nothing from `demo-kit` is in the repository, so no branch or worktree contains it and the
   resets never touch it. Open this script from `~/demos/demo-kit/csharp-feequote` on the
...
4. Check the SDK in the terminal you will run Copilot from:

   ```bash
   dotnet --version
   ```

   Expected: a version that starts with `10.`.

   This matters. `AGENTS.md` tells the agent to run `dotnet test`,
   `dotnet format --verify-no-changes` and `dotnet build -warnaserror` by itself. If an older
   SDK comes first on the PATH, it cannot build `net10.0` projects and the agent's checks fail
   for a reason that has nothing to do with the lesson. The first build also restores xunit
   from NuGet, so run step 5 while you are online.
...
5. Check everything is green:
   ...
   Expected: `Test run summary: Passed!` with `total: 54` and `failed: 0`; no output from
   `dotnet format`; and `0 Warning(s)` and `0 Error(s)` from the build.

6. Open the folder in VS Code and let C# Dev Kit load `FeeQuote.sln`. On `main`, confirm the
   Copilot Chat agent picker lists `reviewer`.

7. Every prompt and command in this script is already in
   `~/demos/demo-kit/csharp-feequote/prompts.txt`, in the order you need them, so nothing is
...
line again. If `bin/` or `obj/` files stay locked after every session is closed, run `dotnet
build-server shutdown` and try again. If a worktree folder still remains, delete it and run
`git worktree prune`. `dotnet restore` brings the restored packages in `obj/` back in line with
the project files if an agent added or removed a package reference. `git clean -fdq` leaves the
ignored `bin/` and `obj/` folders alone, so the next build stays fast. The last line must print
`Passed!` with `total: 54` and `failed: 0`, nothing from `dotnet format`, and `0 Warning(s)`
and `0 Error(s)` from the build; if a check fails with a stale build output, delete the `bin/`
and `obj/` folders under `src` and `tests` and run `dotnet restore` again.
...
Only if the history itself is damaged: `bash ../demo-kit/csharp-feequote/setup-demo.sh --force`
(Windows: `..\demo-kit\csharp-feequote\setup-demo.ps1 -Force`) rebuilds it. It refuses to run
...
if the chat box also shows a target picker, choose **Local**). Hover the context window control
in the chat box so the audience sees the token count before you send anything.

**Paste:**

```
#file:README.md #file:src/FeeQuote/Core/Pricing.cs #file:src/FeeQuote/Legacy/Fees.cs #file:src/FeeQuote/Services/TransferService.cs #file:tests/FeeQuote.Tests/TransferServiceTests.cs
...
- New code should call `Pricing.ComputeFee(amountCents)` for the domestic fee, or
  ...
  `src/FeeQuote/Core/Pricing.cs`.
- New code should **not** call `Fees.CalculateFee` in `src/FeeQuote/Legacy/Fees.cs`, because
  it truncates instead of rounding half to even and ignores the minimum and the cap.
- The README is out of date: it recommends `CalculateFee`.
- Evidence: `Quote_UsesHalfEvenRounding_RegressionTicket4821` in
  `tests/FeeQuote.Tests/TransferServiceTests.cs` expects a fee of 1112 for 123500 cents,
...
```
@README.md @src/FeeQuote/Core/Pricing.cs @src/FeeQuote/Legacy/Fees.cs @src/FeeQuote/Services/TransferService.cs @tests/FeeQuote.Tests/TransferServiceTests.cs
...
**Expected:** the answer names `Quote_UsesHalfEvenRounding_RegressionTicket4821` in
`tests/FeeQuote.Tests/TransferServiceTests.cs` (1112 expected, the legacy helper gives 1111)
and `ProductionCode_OutsideLegacyAndReconciliation_DoesNotReferenceLegacyFeeHelper` in
`tests/FeeQuote.Tests/ArchitectureTests.cs` (it fails when new code references
`CalculateFee`). It may also name `Run_QuoteCommand_PrintsQuoteAndReturnsZero` in
`tests/FeeQuote.Tests/CliTests.cs`, which expects 1112 as well.
...
Expected: `DomesticPolicy` in `src/FeeQuote/Core/Pricing.cs` (cap 2500 cents, written
`capCents: 2_500`), covered by `DomesticPolicy_HasDocumentedLimits` and the "cap applies"
case of `ComputeFee_MatchesWorkedExamples` in `tests/FeeQuote.Tests/PricingTests.cs`.
...
- **What this shows:** a lower tier is fine for mechanical work, but on a task where the
  ...
  passes every existing check, and charges a customer twice.
...
2. Open two new terminals side by side (see "Two terminals side by side" at the top). New
   ...
   In the right one, the same with `feequote-higher` in place of `feequote-lower`. Run
   `dotnet build` once in each worktree, so the agent's first check does not wait for a
   package restore and a cold build.
...
to the client in `src/FeeQuote/Clients/Retry.cs`. What matters is *where* the retry goes.
Every submission carries an idempotency key, and the gateway treats submissions that share a
key as one payment. `PaymentClient.Submit` calls `Send`, which mints a fresh key and calls the
...
(so does wrapping `_client.Submit(transfer)` in `TransferService.Submit`). When the gateway
takes the payment but the response is lost (a `GatewayTimeoutException`), the retry becomes a
second payment. The correct change mints the key once, before the retry, and retries only
`GatewayTimeoutException` (a `GatewayDeclinedException` is final). By default `Retry.WithRetry`
retries every `PaymentGatewayException`, so a plain wrap retries declines too; a correct run
can pass the helper's filter, `shouldRetry: ex => ex is GatewayTimeoutException`. Both versions pass
`dotnet test`, `dotnet format` and the build. The check in
`~/demos/demo-kit/csharp-feequote/tier-check/TierCheckTests.cs` (outside the repository, so no
...
`src/FeeQuote/Clients/PaymentClient.cs` (or `TransferService.Submit` in
`src/FeeQuote/Services/TransferService.cs`, which calls the client), and both report green
checks. Typically:

- The **lower tier** either wraps the `Send` call (or `_client.Submit`), so every attempt gets
  a new idempotency key, or mints the key once but retries every `PaymentGatewayException`, so
  a declined transfer is sent three times. It sometimes commits its change as well.
...
repository: 2 of 2 `claude-haiku-4.5` runs failed (both charged twice and sent a declined
transfer three times), 2 of 2 `claude-opus-5` runs passed.
...
```
!cp ../demo-kit/csharp-feequote/tier-check/TierCheckTests.cs tests/FeeQuote.Tests/ && dotnet test --filter-class FeeQuote.Tests.TierCheckTests
```

**Expected:** the test project builds (a few seconds), then three tests run against a fake
gateway that deduplicates on the idempotency key. Each failure prints
`failed FeeQuote.Tests.TierCheckTests.<test>`, the message below and a short stack trace; the
summary shows `total: 3` (the CLI may also report exit code 2, which means a test failed).

| Test | What it checks | Plain wrap of `Send` | Key minted once |
|---|---|---|---|
| `Submit_RecoversWhenTheGatewayTimesOutOnce` | a retry exists | passes | passes |
| `Submit_ChargesTheCustomerOnceWhenAResponseIsLost` | the gateway took the payment, the response was lost | **fails**: `customer charged 2 times for tr-4300 (keys: ..., ...)` | passes |
| `Submit_DoesNotResubmitADeclinedTransfer` | a decline is final | **fails** if every `PaymentGatewayException` is retried: `declined transfer sent 3 times` | passes |
...
`FeeQuote.Clients.GatewayTimeoutException`, because nothing retries yet.
...
**Say:** "Both versions passed every check the repository had. One of them charges a customer
...
**Expected:** `DomesticFee` defined in `src/FeeQuote/Core/Pricing.cs`, callers updated in
`src/FeeQuote/Services/TransferService.cs` and `tests/FeeQuote.Tests/PricingTests.cs`, and
`ComputeFee` kept as an alias marked `[Obsolete]` that calls `DomesticFee`. The checks still
pass. Both projects treat warnings as errors, so any caller left on the alias fails the build
with `error CS0618: 'Pricing.ComputeFee(long)' is obsolete`; the agent has to move every
...
agent's edits, any commit it made, and the copied check file. `bin` and `obj` are ignored by git, so they are kept.
...
- The check does not compile (for example `error CS1729` or `CS0246`): the run removed the
  `PaymentClient(GatewayTransport)` constructor or renamed an exception type. Read the error
  aloud; that run changed the client's public surface, which is a finding too.
...
| `tests/FeeQuote.Tests/ArchitectureTests.cs` | passes | fails with the ticket 4821 message |
...
3460 and 5000 (usually 3459 and 7500), and then hits a failing `ArchitectureTests` ("New
code must not call the legacy fee helper. Use core pricing (PriceWithPolicy or ComputeFee)
instead. See ticket 4821."). It may then take extra turns, change numbers by hand, or edit a
test.

**Point at, while it runs:**

1. The files it opens before its first edit. Did it read
   `tests/FeeQuote.Tests/TransferServiceTests.cs` or
   `tests/FeeQuote.Tests/ArchitectureTests.cs` first?
2. Whether it calls into `src/FeeQuote/Legacy/Fees.cs`.

**Check when it finishes** (paste into the session):

```
!dotnet run --project src/FeeQuote -- quote --amount 247100 --from AU --to NZ
```
...
tests/FeeQuote.Tests/ArchitectureTests.cs", and let it revise the plan.
...
- **What this shows:** tests, formatters and analyzers are feedback the agent can run
...
the run does not stop for an approval on each one (all three checks are `dotnet` commands):

```bash
copilot --allow-tool "shell(dotnet:*)"
```
...
**Expected:** `Test run summary: Passed!` with `total: 54` and `failed: 0`. The build that
`dotnet test` runs first treats warnings as errors, so it compiled cleanly too.

**Paste:**

```
!dotnet run --project src/FeeQuote -- quote --amount 500000 --from AU --to AU
```

**Expected:** a JSON quote showing `"feeCents": 2500` (the cap) and a single
`Domestic fee` line in `breakdown`.

**Point at:** every test passes, the build is clean and the quote is right. Nothing on screen
says that the last commit, `refactor: tidy transfer service usings`, broke a house rule.
...
**Expected:** the agent runs three commands. `dotnet test` and `dotnet build -warnaserror`
pass; `dotnet format --verify-no-changes` exits with code 2 and reports two problems, both in
`src/FeeQuote/Services/TransferService.cs` (each line starts with the full file path and ends
with the project path in brackets):

- `TransferService.cs(1,1): error IMPORTS: Fix imports ordering.` (`using FeeQuote.Models;`
  sits above `using FeeQuote.Clients;`).
- `TransferService.cs(66,2): error FINALNEWLINE: Fix final newline. Insert '\r\n'.` (the file
  ends without a newline; on macOS and Linux the message may show `'\n'`).

It runs `dotnet format` (or moves the `using` and adds the newline by hand), and runs all
three checks again: green.

**Point at:** the `dotnet format` output. Neither problem failed a test or the build.

**Say:** "Green tests are not the same as done. The formatter catches drift that no test and
no compiler ever sees; it would not catch run A's wrong helper, because that code was
formatted perfectly. You need both."
...
**Expected:** the agent changes `DomesticPolicy` in `src/FeeQuote/Core/Pricing.cs`, runs the
test suite without being asked, and may see up to two failures (it may also update some tests
before it runs them):

- `ComputeFee_MatchesWorkedExamples(caseName: "cap applies", ...)` in
  `tests/FeeQuote.Tests/PricingTests.cs` (`cap applies: expected 2500 but got 3000`)
- `DomesticPolicy_HasDocumentedLimits` in `tests/FeeQuote.Tests/PricingTests.cs` (expects
  2500)
...
- `.github/copilot-instructions.md`: point first at the line "Run `dotnet test`,
  `dotnet format --verify-no-changes` and `dotnet build -warnaserror` before declaring a task
  complete. A task with failing checks is not done." That is the rule you just watched the
  agent follow. Then read the legacy helper line aloud: with it in place, run A would not have
  gone the way it did.
...
- Usings out of order, or a file with no final newline, that passes every test and the build
  and lands anyway because nobody ran the formatter in CI; every later diff of that file
  carries the noise.
...
not, edit `src/FeeQuote/Core/Pricing.cs` and set `capCents: 3_000` without touching the
tests.
...
```
Review the uncommitted change to the domestic cap in src/FeeQuote/Core/Pricing.cs and the tests that cover it
```

**Expected:** a review of the cap change in `src/FeeQuote/Core/Pricing.cs` and the updated
tests, in this order: correctness risks, convention violations, missing tests, readability.
It ends with a one-line verdict (approve, approve with nits, or request changes). It does not
edit any file.
...
```bash
copilot --agent reviewer -p "Review the uncommitted change to the domestic cap in src/FeeQuote/Core/Pricing.cs and the tests that cover it"
```
...
**Expected:** the agent runs `bash scripts/release-notes.sh v0.1.0 v0.2.0` on macOS and
Linux, or `pwsh scripts/release-notes.ps1 v0.1.0 v0.2.0` on Windows (`pwsh` must be on the
PATH), and turns the output into this shape:
...
The script itself also prints a `Commits v0.1.0..v0.2.0` line and an empty "Other" group,
which the agent usually drops.
...
- `src/FeeQuote/Core/Pricing.cs`: `DomesticPolicy` defined (around line 44) and used by
  `ComputeFee` (around line 105).
- `src/FeeQuote/Services/TransferService.cs`: `TransferService.Quote` calls
  `Pricing.ComputeFee` (around line 24), and `transfer.Channel` is available there.
- `src/FeeQuote/Cli.cs`: reaches it through `new TransferService(new PaymentClient()).Quote`
  (around line 42).
- `tests/FeeQuote.Tests/PricingTests.cs`: asserts the policy values.
- `src/FeeQuote/Jobs/Reconciliation.cs` does not read it; it uses the legacy helper.
...
time belongs in a test, an analyzer or an instruction file."
...
- `src/FeeQuote/Services/TransferService.cs` (`TransferService.Quote`)
- `tests/FeeQuote.Tests/PricingTests.cs`
- the definition in `src/FeeQuote/Core/Pricing.cs`

It may also mention `tests/FeeQuote.Tests/ArchitectureTests.cs` or the instruction files, which
contain the name in text or messages; none of those are callers. After the answer comes a
...
- The answer lists files under `bin`, `obj` or cache folders: the agent searched with
...
- [ ] `dotnet --version` prints a 10.x SDK in the terminal Copilot runs from, and a bare
      `dotnet test` prints `total: 54` and `failed: 0` there.
- [ ] `dotnet test`, `dotnet format --verify-no-changes` and `dotnet build -warnaserror` all
      pass on `main` and on `demo-start`.
- [ ] On `demo-checks`, `dotnet test` passes all 54 tests and `dotnet build -warnaserror`
      passes, while `dotnet format --verify-no-changes` reports `IMPORTS` and `FINALNEWLINE`
      in `src/FeeQuote/Services/TransferService.cs`.
...
- [ ] The repository folder contains no `DEMO_SCRIPT.md`, `prompts.txt` or `RUN_COMPARISON.md`;
      they are open from `~/demos/demo-kit/csharp-feequote` on a monitor that is not shared,
      outside the demo VS Code window.
- [ ] In VS Code, the agent's terminal finds the .NET 10 SDK: run B's `dotnet test` and the
      release-notes skill's script (`bash scripts/release-notes.sh` on macOS and Linux,
      `pwsh scripts/release-notes.ps1` on Windows) both run.
...
- [ ] Whether `/model` keeps its choice between sessions: ______ (the script sets it each time
      either way).
- [ ] `dotnet run --project src/FeeQuote -- quote --amount 123500 --from AU --to AU` prints
      fee 1112.
- [ ] Segment 4.3: both worktrees exist and build, both CLI sessions are started, trusted,
      and set to their models. Lower-tier model: ______ Higher-tier model: ______
- [ ] `~/demos/demo-kit/csharp-feequote/tier-check/TierCheckTests.cs` is on disk, and on an
      untouched `demo-start` worktree the segment 4.3 check fails its first two tests and passes
      the third.
...
- [ ] `copilot --agent reviewer -p "Review the uncommitted change to the domestic cap in src/FeeQuote/Core/Pricing.cs and the tests that cover it"` runs with the cap change in
...
- [ ] `bash scripts/release-notes.sh v0.1.0 v0.2.0` on macOS and Linux (Windows:
      `pwsh scripts/release-notes.ps1 v0.1.0 v0.2.0`) prints four commits: four commit bullet
      lines, two under Features and two under Chores (the `(none)` lines under Fixes and Other
      are not commits). On Windows this needs PowerShell 7.
