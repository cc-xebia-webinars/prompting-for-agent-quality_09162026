# FeeQuote demo script (ASP.NET Core MVC + React)
...
- **Terminal** means a window of your terminal app, in the repository folder, where `dotnet`
  and `npm` both run:
  ...
  - **Windows:** Windows Terminal running PowerShell 7 (`pwsh`). Windows PowerShell 5.1 is
    not enough: several commands below chain with `&&`, which 5.1 cannot parse. Copilot CLI
    also runs `!` commands in PowerShell 7 when it is installed.
- **Commands** are the same on all three systems unless the script shows a separate
  **macOS and Linux** block and a **Windows** block. Paths are written as `~/demos/...`; in
  PowerShell `~` is your user folder too, so `cd ~/demos/aspnet-react-feequote` works
  everywhere.
...
1. Install git, the .NET SDK 10, Node.js 24 or later (with npm), VS Code with the GitHub
   Copilot and Copilot Chat extensions, and Copilot CLI.
   - **macOS:** `brew install git node` and `brew install --cask dotnet-sdk copilot-cli` (and
     the Xcode command line tools if git asks for them). Open a new terminal afterwards so
     `dotnet` is on the path.
   - **Linux:** git from your package manager; the .NET SDK 10 (for example
     `sudo apt install dotnet-sdk-10.0` on Ubuntu, or Microsoft's instructions for your
     distribution); Node.js 24 or later from <https://nodejs.org>, NodeSource or nvm
     (distribution packages are often older); then `npm install -g @github/copilot`.
   - **Windows:** `winget install Git.Git Microsoft.DotNet.SDK.10 OpenJS.NodeJS.LTS GitHub.Copilot Microsoft.PowerShell`.
2. Copy two folders out of any synchronized folder (OneDrive, iCloud Drive, Dropbox) into
   `~/demos` (on Windows that is `C:\Users\<you>\demos`), side by side: the
   `aspnet-react-feequote` repository and the `demo-kit` folder. The repository holds only the
   demo code. `demo-kit` holds this script, `prompts.txt`, `RUN_COMPARISON.md`, the setup
   scripts, the segment 4.3 check and `run-stats.py`, outside the repository, so no agent can
   read them. The commands below assume `~/demos/aspnet-react-feequote` and `~/demos/demo-kit`;
   if yours differ, change `~/demos` wherever it appears. The worktrees in segment 4.3 sit next
   to the repository, and `bin`, `obj` and `node_modules` folders nest deeply, so on Windows a
   long path can hit the path length limit.
3. In a terminal in that folder, run:
   ...
   npm ci
   cd ../../..
   bash ../demo-kit/aspnet-react-feequote/setup-demo.sh
   dotnet restore
   cd src/FeeQuote.Web/ClientApp
   npm run build
   cd ../../..
   ...
   npm ci
   cd ../../..
   ..\demo-kit\aspnet-react-feequote\setup-demo.ps1
   dotnet restore
   cd src/FeeQuote.Web/ClientApp
   npm run build
   cd ../../..
   ...
   `pwsh -ExecutionPolicy Bypass -File ..\demo-kit\aspnet-react-feequote\setup-demo.ps1`.
   ...
   resets never touch it. Open this script from `~/demos/demo-kit/aspnet-react-feequote` on
   the unshared monitor, not in the demo VS Code window: it contains the expected answers, and
   an open editor tab is attached to chats as context.

4. Keep `node_modules` installed in `src/FeeQuote.Web/ClientApp` and run Copilot from a
   terminal where `dotnet` and `npm` both work.

   This matters. `AGENTS.md` tells the agent to run `dotnet test`,
   `dotnet format --verify-no-changes`, `dotnet build -warnaserror` and the ClientApp
   `npm run check` by itself. Without `npm install`, the ClientApp checks fail for a reason
   that has nothing to do with the lesson. `node_modules` is ignored by git, so it survives
   every branch switch and reset in this script.
...
5. Check everything is green:
   ...
   npm --prefix src/FeeQuote.Web/ClientApp run check
   ```

   Expected: `Test run summary: Passed!` with `total: 49`, no output from `dotnet format`,
   `0 Warning(s)` and `0 Error(s)`, and `Tests  10 passed (10)` followed by a Vite build that
   ends in `built in`.

   Then start the site with `dotnet run --project src/FeeQuote.Web`, open
   <http://localhost:5080/app/>, get one quote, and stop the site with Ctrl+C. Never leave the
   site running while an agent works: on Windows the running site locks
   `src/FeeQuote.Web/bin`, and the agent's next build fails with `MSB3027 ... The file is
   locked by: "FeeQuote.Web"`.
...
7. Every prompt and command in this script is already in
   `~/demos/demo-kit/aspnet-react-feequote/prompts.txt`, in the order you need them, so
   nothing is typed live. Open it in a plain text editor on the unshared monitor (TextEdit on macOS,
   Notepad on Windows, any text editor on Linux), **not** in the demo VS Code window, where an
   open file is attached to chats.
...
Then type `/exit` in every open Copilot CLI session, close the two segment 4.3 terminals, and
stop the site with Ctrl+C if `dotnet run` is still running (Windows cannot remove a worktree
folder that a running program is using). Then run these in a terminal in the repository folder:
...
dotnet restore
npm --prefix src/FeeQuote.Web/ClientApp ci
git status --short
git worktree list
dotnet test; dotnet format --verify-no-changes; dotnet build -warnaserror
npm --prefix src/FeeQuote.Web/ClientApp run check
```
...
print "Permission denied" instead, a session or a running site is still open in that folder:
close it and run the line again. If a worktree folder still remains, delete it and run `git
worktree prune`. `npm ci` reinstalls `node_modules` from `package-lock.json`, which removes any
package an agent installed. The last two lines must print `Test run summary: Passed!` with
`total: 49`, no output from `dotnet format`, `0 Warning(s)` and `0 Error(s)`, and `Tests 10
passed (10)` followed by a Vite build that ends in `built in`.
...
If the site is still running, stop it with Ctrl+C first.

To throw away one experiment without resetting branches: `git checkout -- . && git clean -fdq`.

Only if the history itself is damaged:
`bash ../demo-kit/aspnet-react-feequote/setup-demo.sh --force` (Windows:
`..\demo-kit\aspnet-react-feequote\setup-demo.ps1 -Force`) rebuilds it, and it refuses to run
unless `main` is checked out at `v0.2.0`, clean and has no extra worktrees. Close VS Code and
every Copilot CLI session, run the full rehearsal reset above, then `git checkout -q main`
before the rebuild. The rebuild keeps `node_modules`, so there is no need to run `npm ci`
again.

If you did the try-it-yourself exercise at the end of this script and its worktrees still
exist, remove them too (`git worktree remove --force ../feequote-api` and
`git worktree remove --force ../feequote-ui`); otherwise `git worktree list` shows more than one
line and a rebuild refuses to run. A rebuild deletes every branch, including `fx-fee`, so
create `fx-fee` again before repeating that exercise.
...
**Before you start:** VS Code is open on the repository, with no editors open. If the site is
still running, stop it with Ctrl+C first. Then, in a terminal, run the live reset:

```bash
cd ~/demos/aspnet-react-feequote
git checkout -q -f demo-start && git reset -q --hard v0.1.0 && git clean -fdq
git status --short
```
...
if the chat box also shows a target picker, choose **Local**).
...
```
#file:README.md #file:src/FeeQuote.Web/Domain/Core/Pricing.cs #file:src/FeeQuote.Web/Domain/Legacy/Fees.cs #file:src/FeeQuote.Web/Domain/Services/TransferService.cs #file:tests/FeeQuote.Web.Tests/TransferServiceTests.cs
...
- New code should call `Pricing.ComputeFee(amountCents)` for the domestic fee, or
  ...
  `src/FeeQuote.Web/Domain/Core/Pricing.cs`.
- New code should **not** call `Fees.CalculateFee` in `src/FeeQuote.Web/Domain/Legacy/Fees.cs`,
  because it truncates instead of rounding half to even and ignores the minimum and the cap.
- The README is out of date: it recommends `Fees.CalculateFee`.
- Evidence: `Quote_RegressionTicket4821_RoundsHalfToEven` in
  `tests/FeeQuote.Web.Tests/TransferServiceTests.cs` expects a fee of 1112 for 123500 cents,
  where the legacy helper would return 1111.
...
```
@README.md @src/FeeQuote.Web/Domain/Core/Pricing.cs @src/FeeQuote.Web/Domain/Legacy/Fees.cs @src/FeeQuote.Web/Domain/Services/TransferService.cs @tests/FeeQuote.Web.Tests/TransferServiceTests.cs
...
**Expected:** the answer names `Quote_RegressionTicket4821_RoundsHalfToEven` in
`tests/FeeQuote.Web.Tests/TransferServiceTests.cs` (1112 expected, the legacy helper gives
1111) and `ProductionCode_DoesNotCallLegacyFeeHelper` in
`tests/FeeQuote.Web.Tests/ArchitectureTests.cs` (it fails when new code references
`CalculateFee`).
...
Expected: `Pricing.DomesticPolicy` in `src/FeeQuote.Web/Domain/Core/Pricing.cs` (cap 2500
cents), covered by `DomesticPolicy_MatchesPublishedFeeSchedule` and the "cap applies" case of
`ComputeFee_WorkedExamples` in `tests/FeeQuote.Web.Tests/PricingTests.cs`.
...
2. Open two new terminals side by side (see "Two terminals side by side" at the top). New
   ...
   In the right one, the same with `feequote-higher` in place of `feequote-lower`. The build
   runs once so the agent's first test run does not also pay for a restore and a cold build.

   No `npm install` is needed in the worktrees. The task is back-end only, and `demo-start`
   has no `AGENTS.md`, so neither agent is told to run the ClientApp checks. If you want a run
   that decides to run them anyway to succeed, also run `npm ci` in
   `src/FeeQuote.Web/ClientApp` of each worktree; without it, `npm test` there fails because
...
to the client in `src/FeeQuote.Web/Domain/Clients/Retry.cs`. What matters is *where* the retry
goes. Every submission carries an idempotency key, and the gateway treats submissions that
share a key as one payment. `PaymentClient.Submit` calls `Send`, which mints a fresh key and
calls the transport in one line:
...
retries only `GatewayTimeoutException` (a `GatewayDeclinedException` is final). Both
exceptions derive from `GatewayUnavailableException`, which is what `WithRetry` retries by
default, so a plain wrap also resends declines; a correct run can pass the helper's filter,
`shouldRetry: ex => ex is GatewayTimeoutException`. Both versions pass the existing suite. The check in
`~/demos/demo-kit/aspnet-react-feequote/tier-check/TierCheckTests.cs` (outside the repository,
so no agent can see it) tells them apart.
...
`src/FeeQuote.Web/Domain/Clients/PaymentClient.cs`, and both report green tests. Typically:

- The **lower tier** either wraps the `Send` call, so every attempt gets a new idempotency
  key, or mints the key once but retries every `GatewayUnavailableException`, so a declined
...
it. Each Haiku run used 5 to 8 AI credits and each Opus run 47 to 68. A smaller trial on this
repository: 2 of 2 `claude-haiku-4.5` runs failed (one charged twice, both resent a declined
transfer), 2 of 2 `claude-opus-5` runs passed.
...
```
!cp ../demo-kit/aspnet-react-feequote/tier-check/TierCheckTests.cs tests/FeeQuote.Web.Tests/ && dotnet test --filter-class FeeQuote.Web.Tests.TierCheckTests
```

**Expected:** three tests against a fake gateway that deduplicates on the idempotency key. The
output ends with `Test run summary:` and `total: 3`; each failure prints its test name and, on
the next line, the message below (followed by a stack trace).

| Test | What it checks | Plain wrap of `Send` | Key minted once |
|---|---|---|---|
| `Submit_GatewayTimesOutOnce_Recovers` | a retry exists | passes | passes |
| `Submit_ResponseLost_ChargesCustomerOnce` | the gateway took the payment, the response was lost | **fails**: `customer charged 2 times for tr-4300 (keys: ..., ...)` | passes |
| `Submit_Declined_DoesNotResubmit` | a decline is final | **fails** if every `GatewayUnavailableException` is retried: `declined transfer sent 3 times` | passes |
...
show both. On an untouched `demo-start` the first two tests fail with
`GatewayTimeoutException`, because nothing retries yet.
...
**Expected:** `DomesticFee` defined in `src/FeeQuote.Web/Domain/Core/Pricing.cs`, callers
updated in `src/FeeQuote.Web/Domain/Services/TransferService.cs` and
`tests/FeeQuote.Web.Tests/PricingTests.cs`, and a `ComputeFee` alias marked `[Obsolete]` that
calls `DomesticFee`. The checks still pass.

The build treats warnings as errors, so any caller left on the alias stops the build with
`error CS0618: 'Pricing.ComputeFee(long)' is obsolete: ...`. A run that also adds a test for
the alias has to suppress CS0618 around that test.
...
agent's edits, any commit it made, and the copied check file. `bin`, `obj` and `node_modules` are ignored by git, so they are kept.
...
- The rename run stops on CS0618: that is the compiler finding a caller the agent missed. Let
  the agent read the error and fix it.
...
|---|---|---|
| Helper called | `Pricing.PriceWithPolicy(amountCents, 50, Pricing.DomesticPolicy)` | `Fees.CalculateFee(amountCents, 50)` for the FX fee, and sometimes `Fees.CalculateFee(amountCents, 90)` for the domestic fee too |
...
| `tests/FeeQuote.Web.Tests/ArchitectureTests.cs` | passes | fails with the ticket 4821 message |

**Checking the quote:** this stack has no command-line entry point; the quote comes from the
running site. After a run finishes, open a second terminal in the same folder and start the
site:
...
Wait for `Now listening on: http://localhost:5080`. Stop it with Ctrl+C as soon as you have
read the quote: while it runs it locks `src/FeeQuote.Web/bin` on Windows, and any later build
fails with MSB3027.
...
**Expected:** a run that trusts the README uses `Fees.CalculateFee`, gets a total other than
3460 and 5000 (usually 3459 and 7500), and then hits a failing
`ArchitectureTests.ProductionCode_DoesNotCallLegacyFeeHelper` with the message "New code must not call the legacy fee helper. Use core pricing (PriceWithPolicy or
ComputeFee) instead. See ticket 4821." It may then take extra turns, change numbers by hand,
or edit a test.

**Point at, while it runs:**

1. The files it opens before its first edit. Did it read
   `tests/FeeQuote.Web.Tests/TransferServiceTests.cs` or
   `tests/FeeQuote.Web.Tests/ArchitectureTests.cs` first?
2. Whether it calls anything from `src/FeeQuote.Web/Domain/Legacy/Fees.cs`.

**Check when it finishes:** start the site in a second terminal as described above, then paste
the command for your system into the session.

macOS and Linux:

```
!curl -s -X POST http://localhost:5080/api/quotes -H "Content-Type: application/json" -d '{"amountCents":247100,"currency":"AUD","originCountry":"AU","destinationCountry":"NZ","channel":"online"}'
```

Windows (the `!` command runs in PowerShell 7):

```
!Invoke-RestMethod -Method Post -Uri http://localhost:5080/api/quotes -ContentType application/json -Body '{"amountCents":247100,"currency":"AUD","originCountry":"AU","destinationCountry":"NZ","channel":"online"}' | ConvertTo-Json -Depth 5
```

**Expected:** a JSON quote. The correct answer is `feeCents` of `3460` with a separate FX line
in `breakdown`. `3459` or `3458` means it used the legacy helper. Stop the site with Ctrl+C.
...
discards run A's changes. Stop the site if it is running (Ctrl+C in its terminal). Then run the
live reset again, in the same terminal:
...
and says to use `Pricing.PriceWithPolicy` with `Pricing.DomesticPolicy` and a 50 basis point
rate.

**If the plan names `CalculateFee`:** type one correction, for example "Use
Pricing.PriceWithPolicy from core pricing, not the legacy helper; see
tests/FeeQuote.Web.Tests/ArchitectureTests.cs", and let it revise the plan.
...
**Check when it finishes:** start the site in a second terminal, then run the command for your
system in the VS Code terminal.
...
**Expected:** `feeCents` of `3460`. Stop the site with Ctrl+C.
...
green; checks run; legacy helper avoided; files touched; turns and tool calls; tokens; credits.
...
- The agent's build fails with MSB3027 and "The file is locked by: FeeQuote.Web": the site is
  still running from an earlier check. Stop it with Ctrl+C and let the agent run the build
  again.
- The quote command cannot connect: the site is not running, or has not printed
  `Now listening on: http://localhost:5080` yet.
- The quote shows `feeCents` of `2224` with only a domestic line: the site was started before
  the agent's change. Stop it, start it again, and rerun the command.
...
changed-files list) and run **View: Close All Editors**. Make sure the site is stopped. In
your terminal app, in the repository folder:
...
`git status --short` must print nothing. Start the session with `dotnet` and `npm` allowed, so
the run does not stop for an approval on each check:

```bash
copilot --allow-tool "shell(dotnet:*)" --allow-tool "shell(npm:*)"
```
...
```
!npm --prefix src/FeeQuote.Web/ClientApp test
```
...
**Expected:** the last commit, `refactor: tidy quote form submit handler`, changes
`src/FeeQuote.Web/ClientApp/src/App.tsx`: the import becomes
`import { useEffect, useState } from "react";` and the submit handler gains
`console.log("quote request", amountCents);`.

**Point at:** every test passes, yet the commit ships a debug line that writes the amount of
every quote request to the browser console, and imports a hook nothing uses.
...
**Expected:** the agent runs the four checks. `dotnet test`,
`dotnet format --verify-no-changes` and `dotnet build -warnaserror` pass. The ClientApp checks
find two problems, both in `src/FeeQuote.Web/ClientApp/src/App.tsx`:

- `npm run typecheck` (tsc) reports
  `src/App.tsx(1,10): error TS6133: 'useEffect' is declared but its value is never read.`
- `npm run lint` (eslint) reports two errors in `App.tsx`:
  `1:10  error  'useEffect' is defined but never used  @typescript-eslint/no-unused-vars` and
  `37:5  error  Unexpected console statement  no-console`, then `2 problems (2 errors, 0 warnings)`.

`npm run check` stops at the first failing step, typecheck, so the `no-console` error may only
appear after the agent fixes the import and runs the check again. It removes `useEffect` from
the import and deletes the `console.log` line, then runs all the checks again: green.

**Point at:** the tsc and the eslint output. Neither problem failed a test.

**Say:** "Green tests are not the same as done. The type checker catches an unused import;
it would not catch run A's wrong helper, because that had a valid signature. You need both."
...
**Expected:** the agent changes `DomesticPolicy` in `src/FeeQuote.Web/Domain/Core/Pricing.cs`,
runs `dotnet test` without being asked, and may see up to two failures (it may also update
some tests before it runs them), both in `tests/FeeQuote.Web.Tests/PricingTests.cs`:

- `DomesticPolicy_MatchesPublishedFeeSchedule` (expects 2500, actual 3000)
- `ComputeFee_WorkedExamples(caseName: "cap applies", amountCents: 500000, expectedFeeCents: 2500)`
  (expects 2500, actual 3000)

It updates the expected values to 3000 and runs the checks again until they pass.

**Point at:** the moment the agent runs `dotnet test` on its own. Nothing in the prompt asked
it to.
...
- `.github/instructions/tests.instructions.md`: point at
  `applyTo: "tests/**,src/FeeQuote.Web/ClientApp/src/**/*.test.tsx"` at the top. These rules
  load only when test files are involved.
- `AGENTS.md`: the short version, with the exact commands (the three `dotnet` commands and
  the ClientApp `npm run check`).
...
- The agent does not run the suite in Step 3: ask `Is this task complete?` and point at the
  ...
  `!dotnet test` yourself and read the failures aloud.
...
- The agent silences eslint (an `eslint-disable` comment) instead of deleting the
  `console.log` line: ask whether a debug log of every quote amount belongs in production
  code, and let it fix the code instead.
...
- A debug `console.log` and a dead import that pass every test and ship because nobody ran
  the linter or type checker in CI.
...
not, edit `src/FeeQuote.Web/Domain/Core/Pricing.cs` and set `CapCents: 3000` without touching
the tests.
...
```
Review the uncommitted change to the domestic cap in src/FeeQuote.Web/Domain/Core/Pricing.cs and the tests that cover it
```

**Expected:** a review of the cap change in `src/FeeQuote.Web/Domain/Core/Pricing.cs` and the
updated tests, in this order: correctness risks, convention violations, missing tests,
readability. It ends with a one-line verdict (approve, approve with nits, or request changes).
...
```bash
copilot --agent reviewer -p "Review the uncommitted change to the domestic cap in src/FeeQuote.Web/Domain/Core/Pricing.cs and the tests that cover it"
```
...
**Expected:** the agent runs `scripts/release-notes.sh v0.1.0 v0.2.0` (on Windows,
`scripts/release-notes.ps1 v0.1.0 v0.2.0`) and turns the output into this shape:
...
A `### Fixes` heading with "none" may also appear; the script prints `- (none)` under Fixes
because there are no fix commits in that range.
...
- `src/FeeQuote.Web/Domain/Core/Pricing.cs`: `DomesticPolicy` defined (around line 17) and
  used by `ComputeFee` (around line 62).
- `src/FeeQuote.Web/Domain/Services/TransferService.cs`: `TransferService.Quote` calls
  `Pricing.ComputeFee` (around line 28), and `Transfer.Channel` is available there.
- `src/FeeQuote.Web/Controllers/Api/QuotesController.cs`: reaches it through
  `TransferService.Quote` (around line 55), after defaulting the channel (around line 51).
- `tests/FeeQuote.Web.Tests/PricingTests.cs`: asserts the policy values.
- `src/FeeQuote.Web/Domain/Jobs/Reconciliation.cs` does not read it; it uses the legacy helper.
- The React app in `src/FeeQuote.Web/ClientApp` does not read it; it only displays the
...
```bash
git grep -n ComputeFee -- src tests
```
...
- `src/FeeQuote.Web/Domain/Services/TransferService.cs`
- `tests/FeeQuote.Web.Tests/PricingTests.cs`
- the definition in `src/FeeQuote.Web/Domain/Core/Pricing.cs`

It may also mention the name in `AGENTS.md`, `.github/copilot-instructions.md` or the
`ArchitectureTests` failure message; none of those are callers. After the answer comes a
summary with an `AI Credits` line and a `Tokens` line. In a rehearsal on the Python repository
the run took about 8 seconds, one tool call, about 55k input tokens and 7.75 AI credits; your
...
- The answer lists files under `bin`, `obj` or `node_modules`: the agent searched with
...
## Try it yourself: parallel sessions (not part of the live session)

- **Where:** two Copilot CLI sessions side by side, one per git worktree (the VS Code Agents
  window works the same way).
- **Branch:** `fx-fee`, a branch holding a finished FX fee feature, checked out in two
  worktrees on new branches `fx-api` and `fx-ui`.
- **What this shows:** a back-end change and a front-end change made at the same time by two
  sessions that cannot step on each other, then merged with every check green.

**Before you start (preparation):**

1. Run the full rehearsal reset from "Reset between rehearsals" first, so the uncommitted cap
   change from segment 4.7 cannot make `git checkout fx-fee` fail later.

   If `fx-fee` does not exist yet, create it. Run segment 4.5 run B (or any run that reached
   the correct result) from a clean `demo-start`, confirm `dotnet test` passes and the quote for 247100
   cents shows `feeCents` of `3460`, then commit the result on a new branch:

   ```bash
   git checkout -b fx-fee
   git add -A
   git commit -m "feat: add FX fee for international transfers"
   ```

   The rehearsal reset does not delete `fx-fee`, so you only do this once (a rebuild with
   `--force`, or `-Force` on Windows, does).

2. Create one worktree per session:

   ```bash
   git worktree add -b fx-api ../feequote-api fx-fee
   git worktree add -b fx-ui ../feequote-ui fx-fee
   ```

3. The front-end session needs the ClientApp packages in its own worktree (`node_modules` is
   not shared between worktrees). Still in the repository folder:

   ```bash
   npm --prefix ../feequote-ui/src/FeeQuote.Web/ClientApp ci
   ```

   Without it, the front-end session's `npm test` fails with a `vitest` not found error
   (macOS, Linux) or `'vitest' is not recognized` (Windows).

4. Open two new terminals side by side (see "Two terminals side by side" at the top). New
   terminals start in your home folder, so use full paths: in the left one,
   `cd ~/demos/feequote-api`; in the right one, `cd ~/demos/feequote-ui`. In both, type
   `copilot`, answer the folder trust prompt, and pick a different model in each with `/model`.

### Step 1: both CLI sessions, start both tasks at the same time

**Paste** (into the `feequote-api` session):

```
Expose the FX fee as `fxFeeCents` at the top level of the /api/quotes response in addition to the breakdown line, and add an integration test
```

**Paste** (into the `feequote-ui` session):

```
Show the FX fee as its own row in the quote breakdown, styled like the other rows, and add a test
```

**Expected:**

- The back-end session edits `src/FeeQuote.Web/Controllers/Api/QuotesController.cs` (a new
  `FxFeeCents` field on `QuoteResponse`) and `tests/FeeQuote.Web.Tests/QuotesApiTests.cs`, and
  runs `dotnet test`.
- The front-end session works in `src/FeeQuote.Web/ClientApp/src`. `QuoteTable` in `App.tsx`
  already renders every breakdown line as a row with the `fee-line` class, so a careful run
  adds a test to `App.test.tsx` that renders an FX fee row and may change `App.tsx` little or
  not at all. It may also add `fxFeeCents` to `QuoteResponse` in `api.ts`.

**Point at:** each session's worktree path, model and tool calls, and that neither session
waits for the other. Paste `/usage` into both when they finish.

**Paste** (into the `feequote-ui` session):

```
Did you add fxFeeCents to QuoteResponse in api.ts?
```

**Point at:** the answer. A good run keeps the TypeScript type in sync with the back-end
session's contract.

### Step 2: both CLI sessions, commit each branch

**Paste** (into the `feequote-api` session):

```
!git add -A && git commit -m "feat: expose fxFeeCents on the quotes API"
```

**Paste** (into the `feequote-ui` session):

```
!git add -A && git commit -m "feat: show the FX fee row in the quote breakdown"
```

**Expected:** one commit on `fx-api` and one on `fx-ui`. Type `/exit` in both sessions.

### Step 3: terminal, merge and check

**Do:** in a terminal in the repository folder, paste:

```bash
git checkout fx-fee
git merge --no-ff --no-edit fx-api
git merge --no-ff --no-edit fx-ui
dotnet test
npm --prefix src/FeeQuote.Web/ClientApp run check
```

**Expected:** two merge commits, `Test run summary: Passed!` with the new integration test
counted in the total, and `npm run check` green with the new front-end test counted.

**Do:** remove the worktrees and their branches:

```bash
git worktree remove ../feequote-api
git worktree remove ../feequote-ui
git branch -d fx-api fx-ui
```

### If something goes wrong

- The second merge stops on a conflict in `api.ts`: both sessions added `fxFeeCents`. Keep
  the field once, then `git add src/FeeQuote.Web/ClientApp/src/api.ts`,
  `git commit --no-edit`, and run `npm --prefix src/FeeQuote.Web/ClientApp run check` again.
- `git worktree remove` refuses because of modified or untracked files: commit what you want
  to keep, then add `--force`.
- `git worktree remove` reports `Filename too long` on Windows: the folder path is too long
  for `bin`, `obj` or `node_modules`. Delete the leftover worktree folder, then run
  `git worktree prune`.

### Where this shows up in real repositories

- An API field added on the server while the client type is changed separately, so the two
  drift apart until someone notices a missing value on screen.

---

## Rehearsal checklist

- [ ] `node_modules` is installed in `src/FeeQuote.Web/ClientApp`, and in the terminal Copilot
      runs from, `npm --prefix src/FeeQuote.Web/ClientApp test` prints `Tests  10 passed (10)`.
- [ ] `dotnet test` (49 tests), `dotnet format --verify-no-changes`,
      `dotnet build -warnaserror` and `npm --prefix src/FeeQuote.Web/ClientApp run check` all
      pass on `main` and on `demo-start`.
- [ ] On `demo-checks`, the three `dotnet` checks pass (49 tests) and `npm test` in
      `src/FeeQuote.Web/ClientApp` prints `Tests  10 passed (10)`, while `npm run typecheck`
      reports TS6133 for `useEffect` and `npm run lint` reports `no-unused-vars` and
      `no-console` in `src/App.tsx`.
...
- [ ] The repository folder contains no `DEMO_SCRIPT.md`, `prompts.txt` or `RUN_COMPARISON.md`;
      they are open from `~/demos/demo-kit/aspnet-react-feequote` on a monitor that is not
      shared, outside the demo VS Code window.
- [ ] In VS Code, the agent's terminal can run `dotnet` and `npm`: run B's `dotnet test`, the
      ClientApp `npm --prefix src/FeeQuote.Web/ClientApp run check` and the release-notes
      skill's `scripts/release-notes.sh` (Windows: `scripts/release-notes.ps1`) all run there.
...
- [ ] Whether `/model` keeps its choice between sessions: ______ (the script sets it each time
      either way).
- [ ] With the site running, a POST to `/api/quotes` for 123500 cents from AU to AU returns
      `feeCents` of `1112`, and <http://localhost:5080/app/> renders a quote. The site is
      stopped again afterwards.
- [ ] `README.md` "Fee calculation" still points at `Fees.CalculateFee` (do not fix it).
- [ ] Segment 4.3: both worktrees exist and have been built once, both CLI sessions are
      ...
      model: ______
- [ ] `~/demos/demo-kit/aspnet-react-feequote/tier-check/TierCheckTests.cs` is on disk, and on
      an untouched `demo-start` worktree the segment 4.3 check fails its first two tests and
      passes the third.
...
- [ ] `copilot --agent reviewer -p "Review the uncommitted change to the domestic cap in src/FeeQuote.Web/Domain/Core/Pricing.cs and the tests that cover it"` runs with the cap change in
...
- [ ] `scripts/release-notes.sh v0.1.0 v0.2.0` (Windows: `scripts/release-notes.ps1 v0.1.0 v0.2.0`)
      prints four commits.
